...
def dicom_to_image(
    dcm: Dicom, dest: Optional[Path] = None, split: bool = False, fps: int = 5, quality: int = 95, block: bool = True
) -> None:
    data = read_dicom_image(dcm)
...
def main(args: argparse.Namespace) -> None:
    ...
    with pydicom.dcmread(source) as dcm:
        dicom_to_image(dcm, dest, args.split, args.fps, args.quality, not args.noblock)
